...
export class MonitorComponent {
  /*@ngInject*/
  constructor($loading, FeedsResource, LazyCall, myModal, $cookies, $state) {
    let self = this;
    parent = this;
    //services definition
  ...
  searchKeywordSubmit(keyword) {
    ...
    parent.lazyCallService.getPage('monitor', 0, this.tablePerPageNum, tableStateTemp, this.filterData, keyword, function(res) {
  ...
  filterCall(filterData) {
    ...
    if (this.uniqueCallServerFlag) {
      parent.lazyCallService.getPage('monitor', 0, this.tablePerPageNum, tableStateTemp, this.filterData, parent.searchKeyword, function(res) {
        parent.displayed = res.data;
        parent.numberOfItems = res.numberOfItems;
        parent.tableState.pagination.start = 0;
        parent.tableState.pagination.numberOfPages = res.numberOfPages;
  ...
  callServer(tableState) {
    ...
    parent.lazyCallService.getPage('monitor', start, number, tableState, parent.filterData, parent.searchKeyword, function(res) {
      parent.displayed = res.data;
      parent.numberOfItems = res.numberOfItems;
  ...
  removeSubmit(category) {
    ...
    this.feedsResourceService.removeFromMonitor({
      ids: this.selected
    }, function(res) {
  ...
  priorityChange(row) {
      console.log("row====>", row);
    this.feedsResourceService.setPriority({_id: row._id, priority: row.priority},function(result) {
        console.log("priorityChange====>", result);
...
MonitorComponent.$inject = ['$loading', 'FeedsResource', 'LazyCall', 'myModal', '$cookies', '$state'];

export default angular.module('webApp.monitor', [uiRouter])
  .config(routes)
  .component('monitor', {
